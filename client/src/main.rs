    ...
    match opts.command {
        ...
        CommandsName::OpenPosition {
            ...
            if find_position.nft_mint == Pubkey::default() {
                ...
                let signature = send_txn_with_simulate(&rpc_client, &txn, true, true)?;
                println!("{}", signature);
            } else {
        ...
        CommandsName::DecreaseLiquidity {
            ...
            println!("position_nft_infos:{:#?}", position_nft_infos);
            let positions: Vec<Pubkey> = position_nft_infos
                .iter()
                .map(|item| item.position)
                .collect();
        ...
        CommandsName::RemoveAllLiquidity { simulate } => {
            // load pool to get observation
            let pool: raydium_amm_v3::states::PoolState =
                program.account(pool_config.pool_id_account.unwrap())?;
            ...
            let mut find_positions = Vec::new();
            for position in user_positions {
                if position.pool_id == pool_config.pool_id_account.unwrap()
                // && position.tick_lower_index == tick_lower_index
                // && position.tick_upper_index == tick_upper_index
                {
                    let find_position = position.clone();
                    println!("liquidity:{:?}", &find_position);
                    find_positions.push(find_position);
                }
            }

            for find_position in find_positions {
                // get position user nft token info
                let user_nft_token_info = position_nft_infos
                    .iter()
                    .find(|&nft_info| nft_info.mint == find_position.nft_mint)
                    .unwrap();
